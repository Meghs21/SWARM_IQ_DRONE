...
| :-: | :--- | :--- | :--- |
| **1** | **Boids Flocking (Reynolds Model)** | Local Swarm Coordination | Vectorized compute of **Separation** ($\vec{F}_{\text{sep}} = \sum \frac{\vec{r}_i - \vec{r}_j}{d_{ij}^2}$), **Alignment** ($\vec{F}_{\text{ali}} = \frac{1}{k}\sum \vec{v}_j - \vec{v}_i$), and **Cohesion** ($\vec{F}_{\text{coh}} = \vec{r}_{\text{center}} - \vec{r}_i$). Operates within $k$-nearest neighbor graph in $<2\text{ms}$. |
| **2** | **Dynamic Leader Election** | Fleet Hierarchy & Resilience | Multi-criteria scoring: $\text{Score}_i = 0.35 \cdot B_i + 0.30 \cdot (1 - \frac{d_{\text{goal}}}{D}) + 0.20 \cdot S_i + 0.15 \cdot H_i$. If the leader is faulted or drops below 20% battery, the highest-ranking active follower seamlessly assumes command without stopping the mission. |
| **3** | **3D A\* Path Planning** | Global Obstacle Navigation | Evaluates $f(n) = g(n) + h(n)$ across a voxelized 3D grid with safety boundary inflation. Computed **only for the active leader**, followed by line-of-sight raycast string pulling to eliminate zigzag waypoints. Followers track formation slots, saving massive CPU cycles. |
| **4** | **Artificial Potential Fields (APF)** | Local Obstacle Avoidance | Combines attractive goal force ($\vec{F}_{\text{att}} = k_{\text{att}}(\vec{x}_{\text{goal}} - \vec{x})$) and repulsive obstacle field ($\vec{F}_{\text{rep}} = k_{\text{rep}} (\frac{1}{d} - \frac{1}{d_0})\frac{1}{d^2}\hat{n}$). Enhanced with **tangential vortex curl** ($\vec{F}_{\text{curl}} = \hat{n} \times \hat{u}_{\text{up}}$) to guide the swarm smoothly around high-rise obstacles without getting trapped in local minima. |
| **5** | **Formation Controller** | Tactical Geometry Morphing | Computes relative coordinate offsets rotated by the leader's 3D forward heading: $\vec{p}_i = \vec{p}_{\text{leader}} + \mathbf{R}(\theta)\vec{o}_i$. Supports real-time morphing between **V-Formation**, **Circle**, **Line**, and **Grid**, with dynamic spacing scaling based on swarm size. |
| **6** | **Drone-to-Drone Collision Avoidance & Hard Hull** | Inter-Agent Safety & Zero-Collision | Combines hyperbolic inter-agent repulsion with dynamic formation force attenuation ($\text{form\_attenuation} \to 0$ when in proximity danger). Augmented with a physical **$0.95\text{ m}$ hard-core clearance hull** and relative velocity damping, mathematically guaranteeing **0 actual collisions** across all scenarios. |
