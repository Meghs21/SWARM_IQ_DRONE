...
| :-: | :--- | :--- | :--- |
| **1** | **Boids Flocking (Reynolds Model)** | Local Swarm Coordination | Vectorized computation of **Separation** ($\vec{F}_{\mathrm{sep}} = \sum_{j \in N_i} \frac{\vec{r}_i-\vec{r}_j}{d_{ij}^{2}}$), **Alignment** ($\vec{F}_{\mathrm{ali}} = \frac{1}{k}\sum_{j \in N_i}(\vec{v}_j-\vec{v}_i)$), and **Cohesion** ($\vec{F}_{\mathrm{coh}} = \vec{r}_{\mathrm{center}}-\vec{r}_i$). Operates over a $k$-nearest-neighbor graph for efficient local coordination. |
| **2** | **Dynamic Leader Election** | Fleet Hierarchy & Resilience | Multi-criteria scoring: $\mathrm{Score}_i = 0.35B_i + 0.30\left(1-\frac{d_{\mathrm{goal}}}{D}\right) + 0.20S_i + 0.15H_i$. If the leader fails or its battery drops below **20%**, the highest-ranked active follower assumes leadership without stopping the mission. |
| **3** | **3D A* Path Planning** | Global Obstacle Navigation | Evaluates $f(n)=g(n)+h(n)$ across a voxelized **3D grid** with safety-boundary inflation. Path planning is computed only for the active leader, followed by **line-of-sight raycast string pulling** to remove unnecessary zigzag waypoints. Followers track formation slots instead of independently planning global paths, reducing computational overhead. |
| **4** | **Artificial Potential Fields (APF)** | Local Obstacle Avoidance | Combines an attractive goal force ($\vec{F}_{\mathrm{att}}=k_{\mathrm{att}}(\vec{x}_{\mathrm{goal}}-\vec{x})$) with a repulsive obstacle field ($\vec{F}_{\mathrm{rep}}=k_{\mathrm{rep}}\left(\frac{1}{d}-\frac{1}{d_0}\right)\frac{1}{d^2}\hat{n}$). Enhanced with a **tangential vortex component** ($\vec{F}_{\mathrm{curl}}=\hat{n}\times\hat{u}_{\mathrm{up}}$) to guide the swarm smoothly around high-rise obstacles and reduce local-minimum trapping. |
| **5** | **Formation Controller** | Tactical Geometry Morphing | Computes relative position offsets rotated according to the leader's 3D heading: $\vec{p}_i=\vec{p}_{\mathrm{leader}}+\mathbf{R}(\theta)\vec{o}_i$. Supports real-time morphing between **V-Formation, Circle, Line, and Grid**, with dynamic spacing based on swarm size. |
| **6** | **Drone-to-Drone Collision Avoidance & Hard Hull** | Inter-Agent Safety | Combines hyperbolic inter-agent repulsion with **dynamic formation-force attenuation**, where the formation force approaches zero when proximity danger is detected. A **$0.95\,\mathrm{m}$ hard-core clearance boundary** and relative-velocity damping provide an additional safety layer. The implemented system recorded **zero collisions across the tested scenarios**. |
---
